      ...
      <div class="chatroom-start" v-if="!isSubmitted">
        ...
        <div class="user-img-section">
          <div class="img-select" v-for="(i, key) in imgList" :key="i.id">
            <img
              :class="{ active: i.url == activeImage }"
              :src="i.url"
              alt=""
              @click="selectImg(i.url)"
            />
          </div>
        </div>
        <div class="user-content">
          <input type="text" placeholder="請輸入暱稱" v-model="tempUsername" />
          <button @click="submitUsername" class="Btn Btn-dark chatbtn">
            進入
          </button>
        </div>
      </div>
        ...
        <div class="user-section">
          <div class="text-content">
            <div class="input-section">
              ...
              <img
                src="../assets/images/chatroom/Vector.svg"
                alt=""
                @click="addMessage"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    addMessage() {
      ...
      let image =
        this.currentMessageImage || "src/assets/images/user/userimage.png";
    ...
    scrollToBottom() {
      ...
      if (chatContainer) {
        if (chatContainer.scrollHeight > 0) {
          ...
          console.warn(
            "chatContainer.scrollHeight is 0. Cannot scroll to bottom."
          );
        }
      } else {
